...
#define IS_DATASEG(t)	((t) <= CURSEG_COLD_DATA || (t) >= 6)
...
struct seg_entry {
	...
	time64_t acc_time;
	time64_t acc_period_sum;
	time64_t saved_sum;
	//unsigned int circ_period[4];
	//unsigned int circ_idx;
	//unsigned int acc_level;
};
...
struct segment_allocation {
	void (*allocate_segment)(struct f2fs_sb_info *, int, bool, bool, struct f2fs_io_info *, int);
};
...
{
	if (type >= 6)
		return (struct curseg_info *)(SM_I(sbi)->curseg_array + CURSEG_COLD_DATA);
	else
	...
	if (test_and_clear_bit(segno, free_i->free_segmap)) {
		...
		if (next >= start_segno + usable_segs) {
			if (USE_STRIPE && !sbi->no_stripe) {
				if (sbi->sec_occu_info[secno] == 0 && test_and_clear_bit(secno, free_i->free_secmap)){
					free_i->free_sections++;
				}
			}
			else {
