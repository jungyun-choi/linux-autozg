...
static int f2fs_show_options(struct seq_file *seq, struct dentry *root)
{
	...
	if (test_opt(sbi, NOHEAP))
		...
		seq_puts(seq, ",heap");
	if (test_opt(sbi, NOSTRIPE))
		seq_puts(seq, ",no_stripe");
#ifdef CONFIG_F2FS_FS_XATTR
				...
				struct buffer_head *bh)
{
	...
	if (le32_to_cpu(raw_super->log_blocks_per_seg) != 7) {
	//if (le32_to_cpu(raw_super->log_blocks_per_seg) != 5) {
	//if (le32_to_cpu(raw_super->log_blocks_per_seg) != 6) {
	...
	if (segment_count > (le64_to_cpu(raw_super->block_count) >> 7)) {
	//if (segment_count > (le64_to_cpu(raw_super->block_count) >> 5)) {
	//if (segment_count > (le64_to_cpu(raw_super->block_count) >> 6)) {
...
static void init_sb_info(struct f2fs_sb_info *sbi)
{
	...
	if (USE_STRIPE && !sbi->no_stripe) 
		sbi->max_victim_search = sbi->total_sections;
	else
...
static int init_blkz_info(struct f2fs_sb_info *sbi, int devi)
{
	...
	FDEV(devi).nr_blkz = div64_u64(SECTOR_TO_BLOCK(nr_sectors), sbi->blocks_per_blkz);
